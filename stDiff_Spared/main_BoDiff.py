...
from model_BoDiff.BoDiff_model import TransformerCondition
...
def main():
    ...
    train_dataloader = get_data_loader(
        st_data_train[:,:,0], 
        st_data_masked_train[:,:,1:], 
        mask_train[:,:,0],
        batch_size=batch_size, 
        is_shuffle=True)
    
    valid_dataloader = get_data_loader(
        st_data_masked_valid[:,:,0], 
        st_data_masked_valid[:,:,1:],
        mask_valid[:,:,0], 
        batch_size=batch_size, 
        is_shuffle=True)
    ...
    if 'test' in splits:
        ...
        st_data_masked_test[:,:,0], 
        st_data_masked_test[:,:,1:],
        mask_test[:,:,0], 
        batch_size=batch_size, 
        is_shuffle=True)

    ### DIFFUSION MODEL ##########################################################################
    input_size = st_data_train[0].shape
    # Analiza la imagen como un unico parche
    patch_size = input_size 
    # Analizar cada vector de vecinos (cada gen y sus vecinos) como un parche
    #patch_size = (1, neighbors)
    #num_nn = 1
    # Define the model
    """
    if num_nn > 1024:
    ...
    if "deltas" in pred_layer:
        avg_tensor = torch.tensor(adata.var["c_d_log1p_avg_exp"]).view(1, neighbors)
    else:
        avg_tensor = None
    
    model = TransformerCondition(
...
### Train the model
    ...
    if not os.path.isfile(save_path_prefix):
        normal_train_stDiff(model,
                                ...
                                valid_data = st_data_valid[:,:,0],
                                valid_masked_data = st_data_masked_valid[:,:,1:],
    ...
    if "test" in splits:
        ...
        test_metrics, imputation_data = inference_function(dataloader=test_dataloader, 
                                    data=st_data_test[:,:,0], 
                                    masked_data=st_data_masked_test[:,:,1:], 
                                    mask=mask_test[:,:,0],
                                    max_norm = max_test,
                                    avg_tensor = avg_tensor,
                                    model=model,
