...
def log_pred_image_extreme_completion(adata: ad.AnnData, args, epoch, slide = "", gene_id=0):
    """
    ...
    # TODO: Add MSE values for the plots and make them better
    def log_genes_for_slide(slide_adata, diff_layer, gt_layer, gene_id, epoch):
        """
        ...
        gene_min = min([gene_min_gt, gene_min_diff])
        gene_max = max([gene_max_gt, gene_max_diff])
        ...
        sq.pl.spatial_scatter(slide_adata, color=[gene], layer=diff_layer, ax=ax[1], cmap='jet', norm=norm, colorbar=True, title="")
        ...
        # Set titles
        ax[0].set_title('Ground Truth', fontsize='xx-large')
        ax[1].set_title('Diffusion Imputation', fontsize='xx-large')
        
        # Log plot 
        #wandb.log({top_bottom: fig})
        #FIXME: replace this paths for general paths and save the images in w&b
        
        if not os.path.exists(f"results_visualizations/{args.dataset}/"):
            os.makedirs(f"results_visualizations/{args.dataset}/")
        
        fig.savefig(f"results_visualizations/{args.dataset}/{slide}_{gene}_{gene_id}_{str(epoch)}.jpg")
        
    log_genes_for_slide(slide_adata=slide_adata, diff_layer=diff_layer, gt_layer=gt_layer, gene_id=gene_id, epoch=epoch)
