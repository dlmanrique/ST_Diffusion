...
input_file = "/home/dvegaa/ST_Diffusion/stDiff_Spared/csv/metodos_pred_pcc.csv"
...
ax1.set_xlim(0,5)
ax1.legend(title="Training data\ncompleted with:", loc="upper right", fontsize=9)
...
for i, (v1, v2) in enumerate(zip(values_1, values_2)):
    ...
    ha_alignment = "center"
    if i == 0 or i == 1:  # Primer punto (SEPAL)
        ha_alignment = "left"
    elif i == len(x) - 1:  # Último punto (BLEEP)
        ha_alignment = "right"

    ax2.text(x[i], v2 + 0.06, f"{v2*100:.1f}%", ha=ha_alignment, va="center", fontsize=10, color="black")
...
# Show plot
plt.savefig("/home/dvegaa/ST_Diffusion/stDiff_Spared/visualization_results/miccai_plots/pred_plot_pcc.svg", bbox_inches='tight')
