...
def log_genes_for_slide(genes, slide_adata, experiment_name = 'results', set_name = '', is_deltas=False, pretrained_transformer_results=False, model2select_genes='diffusion', metric2select_genes='mse'):
    ...
    for g in genes:         
        ...
        gene_min_spackle = slide_adata[:, g].layers[spackle_layer].min() 
        gene_max_spackle = slide_adata[:, g].layers[spackle_layer].max()
        ...
        # Set PCC spackle
        #pcc_spackle = str(round(slide_adata.var["spackle_pcc_test"][g], 3))
        # Set MSE spackle
        mse_spackle = str(round(slide_adata.var["spackle_mse_test"][g], 3))

        # Define color normalization
        norm = matplotlib.colors.Normalize(vmin=gene_min, vmax=gene_max)
        
        plot_mask = (1-slide_adata.layers["spackle_mask"])
        ...
        # Plot layers
        slide_adata.layers["spackle_mask"] = slide_adata.layers["spackle_mask"].astype(int)
        sq.pl.spatial_scatter(slide_adata, color=[g], layer="gt_masked", fig=fig, ax=ax[row,0], cmap='jet', norm=norm, colorbar=True, title="", na_color="black")
        sq.pl.spatial_scatter(slide_adata, color=[g], layer=gt_layer, fig=fig, ax=ax[row,1], cmap='jet', norm=norm, colorbar=False, title="", na_color="black")
        sq.pl.spatial_scatter(slide_adata, color=[g], layer=pred_layer, fig=fig, ax=ax[row,2], cmap='jet', norm=norm, colorbar=False, title="", na_color="black")
        sq.pl.spatial_scatter(slide_adata, color=[g], layer=spackle_layer, fig=fig, ax=ax[row,3], cmap='jet', norm=norm, colorbar=False, title="", na_color="black")
        
        # Set titles
        #ax[row, 1].set_title(f'PCC = {pcc_diffusion} & MSE = {mse_diffusion}', fontsize='xx-large')
        #ax[row, 2].set_title(f'PCC = {pcc_spackle} & MSE = {mse_spackle}', fontsize='xx-large')
        ax[row, 2].set_title(f'MSE = {mse_diffusion}', fontsize='xx-large')
        ax[row, 3].set_title(f'MSE = {mse_spackle}', fontsize='xx-large')
        
        # Set y labels
        gene_name = slide_adata.var["gene_ids"][int(g)]
        ax[row,0].set_ylabel(f'{gene_name}:\n{slide}\n', fontsize='x-large')
        ax[row,0].set_xticks([])
        ax[row,0].set_yticks([])
        ax[row,1].set_ylabel('')
        ax[row,2].set_ylabel('')
        ...
        ceros_gt = [gene_adata.layers[gt_layer][gene_adata.layers["spackle_mask"]==True]][0]
        ceros_pred = [gene_adata.layers[pred_layer][gene_adata.layers["spackle_mask"]==True]][0]
        ceros_spackle = [gene_adata.layers[spackle_layer][gene_adata.layers["spackle_mask"]==True]][0]

        # Plot gen predictions and ground truth
        color = ["#1a8899", "#a5e1e3"]
        ax[row,4].plot(ceros_gt, ceros_gt, color="black", linestyle="-", label="Ground Truth")
        ax[row,4].plot(ceros_gt, ceros_pred, color=color[1], marker="o",  markersize=3, linestyle="None", label="LGDiST")
        ax[row,4].plot(ceros_gt, ceros_spackle, color=color[0], marker="o",  markersize=3, linestyle="None", label="SpaCKLE")
        ax[row,4].set_xlabel("Ground Truth", fontsize="large")
        ax[row,4].set_ylabel("Prediction", fontsize="large")
        ax[row, 4].legend(loc="upper left", fontsize="large")
    ...
    # Set PCC spackle
    #pcc_spackle = str(round(slide_adata.var["spackle_pcc_test"][genes[0]], 3))
    # Set MSE spackle
    mse_spackle = str(round(slide_adata.var["spackle_mse_test"][genes[0]], 3))

    # Set titles
    ax[0, 0].set_title(f"Masked Map", fontsize='xx-large')
    #ax[0, 1].set_title(f'Diffusion\nPCC = {pcc_diffusion} & MSE = {mse_diffusion}', fontsize='xx-large')
    #ax[0, 2].set_title(f'Spackle\nPCC = {pcc_spackle} & MSE = {mse_spackle}', fontsize='xx-large')
    ax[0, 1].set_title('Ground Truth', fontsize='xx-large')
    ax[0, 2].set_title(f'LGDiST\nMSE = {mse_diffusion}', fontsize='xx-large')
    ax[0, 3].set_title(f'SpaCKLE\nMSE = {mse_spackle}', fontsize='xx-large')

    fig_path = os.path.join('partial_results', experiment_name)
    os.makedirs(fig_path, exist_ok=True)
    fig.savefig(os.path.join(fig_path, f'preds_{experiment_name}_{set_name}_{model2select_genes}_{metric2select_genes}_2.pdf') ,bbox_inches='tight')
        

def plot_pred_image(adata: ad.AnnData, exp_name: str, genes: list = ["7","11","10"], n_genes: int = 3, slide = "", is_deltas=False, pretrained_transformer_results=False, model2select_genes='diffusion', metric2select_genes='mse'):
    """
    ...
    spackle_layer = adata.layers["spackle_pred"]
    ...
    # Add global metrics to adata
    adata.var['spackle_pcc_global'] = spackle_detailed_metrics['PCC-Gene']
    adata.var['spackle_mse_global'] = spackle_detailed_metrics['MSE']
    # Add detalied metrics to adata
    #adata.var['spackle_pcc_test'] = spackle_detailed_metrics['detailed_PCC-Gene']
    adata.var['spackle_mse_test'] = spackle_detailed_metrics['detailed_mse_gene']
    ...
    for g in n_top: 
        gene_adata = slide_adata[:,g].copy()
        true_values = np.count_nonzero(gene_adata.layers['spackle_mask'])
    ...
    log_genes_for_slide(
        ...
        set_name=f"{gene[0]}_{gene[1]}_{gene[2]}",
        is_deltas=is_deltas,
