...
import pathlib
import sys
STDIFF_PATH = pathlib.Path(__file__).resolve().parent.parent
sys.path.append(str(STDIFF_PATH))
from utils import *
...
class ArgumentParser:
    ...
    def _add_arguments(self):
        ...
        self.parser.add_argument('--model2select_genes', type=str, default='spackle', help='Model to select the best  and worst genes')
...
def log_genes_for_slide(genes, slide_adata, experiment_name = 'results', set_name = '', is_deltas=False, pretrained_transformer_results=False, model2select_genes='spackle', metric2select_genes='mse'):
    """
    ...
    for g in genes:         
        ...
        # Set titles
        ax[row, 1].set_title(f'PCC = {pcc_spackle} & MSE = {mse_spackle}', fontsize='xx-large')
        ...
        ax[row,3].plot(ceros_gt, ceros_pred, color="blue", marker="o",  markersize=3, linestyle="None", label="SpaCKLE")
...
def plot_pred_image(adata: ad.AnnData, n_genes: int = 3, slide = "", is_deltas=False, pretrained_transformer_results=False, model2select_genes='spackle', metric2select_genes='mse'):
    """
    ...
    gt_layer = adata.layers['c_d_log1p']
    pred_layer = adata.layers['c_t_log1p'] 
    ...
    gene_len = len(spackle_detailed_metrics['detailed_PCC-Gene'])
...
#path_adata = "/home/dvegaa/ST_Diffusion/stDiff_Spared/processed_data/mirzazadeh_data/mirzazadeh_mouse_brain_p2/2025-06-20-14-06-02/adata.h5ad"
#adata = ad.read_h5ad(path_adata)
#breakpoint()

#plot_pred_image(adata = adata, model2select_genes = 'spackle', metric2select_genes = 'mse')
