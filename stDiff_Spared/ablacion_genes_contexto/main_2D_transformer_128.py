...
def main():
    ...
    mask_extreme = np.zeros((mask_valid.shape[0], mask_valid.shape[1], mask_valid.shape[2]))
    mask_extreme_completion_valid = get_mask_extreme_completion(adata_128[adata_128.obs["split"]=="val"], mask_extreme, genes_evaluate, args)
    ...
    model.to(device)
    save_path_prefix = args.dataset + "_128.pt"
    
    ### Train the model
    model.train()
    if not os.path.isfile(save_path_prefix):
        adata_valid = [adata_128[adata_128.obs["split"]=="val"], adata_128[adata_128.obs["split"]=="val"]]
        adata_train = [adata_128[adata_128.obs["split"]=="train"], adata_128[adata_128.obs["split"]=="train"]]
