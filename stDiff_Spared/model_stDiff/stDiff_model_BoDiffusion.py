...
class OutputProcess(nn.Module):
    def __init__(self, hidden_size, in_channels, nneighbors, ngens, nchannels, out_channels):
        super().__init__()
        ...
        self.linear_1 = nn.Linear(self.hidden_size, self.in_channels)
        self.linear_2 = nn.Linear(self.nchannels, out_channels, bias=True)

    def forward(self, output):
        bs, genes, d = output.shape

        output = self.linear_1(output)  
        output = output.reshape(bs, self.ngens, self.nneighbors, self.nchannels)  # (bs, 128, 7, 3)
        #output = output.reshape(bs, self.nneighbors, self.ngens, self.nchannels) #(bs, 7, 128, 3)
        
        output = self.linear_2(output) # (bs, 7, 128, 1)
        output = output.permute(0, 3, 1, 2)  # (bs, 1, 128, 7)
        #output = output.permute(0, 3, 2, 1)  # (bs, 1, 128, 7)
        return output
...
class DiT_stDiff_BoDiffusion(nn.Module):
    def __init__(self,
                 input_size,
                 hidden_size,
                 depth,
        ...
        self.nchannels = 3
        self.in_channels = self.nchannels*self.input_size[1]
        #self.in_channels = self.nchannels*self.input_size[0]
        ...
        # out layer
        #self.out_layer = FinalLayer(self.hidden_size, self.in_channels)
        self.out_layer = OutputProcess(self.hidden_size, self.in_channels, self.input_size[1], self.input_size[0], self.nchannels, self.out_size)
    ...
    def forward(self, x, t, y,**kwargs): 
        ...
        #breakpoint()
        input = torch.cat((x, cond, mask), dim=1) #(bs, 3, 128, 7)
        input = input.permute(0, 2, 3, 1) #(bs, 128, 7, 3)
        #input = input.permute(0, 3, 1, 2) #(bs, 7, 128, 3)
        input = input.reshape(input.shape[0], input.shape[1], input.shape[2]*input.shape[3]) 
        
        input = self.in_layer(input) #(bs, 128, hidden_size)
