...
class NoiseScheduler():
    ...
    def add_noise(self, x_start, x_noise, timesteps):  # forward
        ...
        s1 = s1.view(len(timesteps), 1, 1, 1).expand(-1, x_start.shape[1], x_start.shape[2], x_start.shape[3]).to(x_start.device)
        #s2 = s2.reshape(-1, 1).to(x_start.device)
        s2 = s2.view(len(timesteps), 1, 1, 1).expand(-1, x_start.shape[1], x_start.shape[2], x_start.shape[3]).to(x_start.device)
